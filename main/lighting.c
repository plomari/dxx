	...
	switch (obj->type)
	{
		case OBJ_PLAYER:
			...
			else if ((Game_mode & GM_HOARD) && Players[obj->id].secondary_ammo[PROXIMITY_INDEX]) // If hoard game and player, add extra light based on how many orbs you have Pulse as well.
			{
				...
				hoardlight++;
				fix_sincos (((fix)(GameTime/2)) & 0xFFFF,&s,NULL); // probably a bad way to do it
				s+=F1_0; 
				s>>=1;
		...
		case OBJ_WEAPON:
		{
			...
			if (obj->id == FLARE_ID )
				light_intensity = 2*(min(tval, obj->lifeleft) + ((((fix)GameTime) ^ Obj_light_xlate[objnum&0x0f]) & 0x3fff));
			else
