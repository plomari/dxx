...
void do_omega_stuff(object *parent_objp, vms_vector *firing_pos, object *weapon_objp)
{
	...
	if (pnum == Player_num) {
		...
		if (GameTime - Last_omega_fire_time + OMEGA_BASE_TIME <= FrameTime) // if firing is prolonged by FrameTime overhead, let's try to fix that. Since Next_laser_firing_time is probably changed already (in do_laser_firing_player), we need to calculate the overhead slightly different.
			fire_frame_overhead = GameTime - Last_omega_fire_time + OMEGA_BASE_TIME;

		Next_laser_fire_time = GameTime+OMEGA_BASE_TIME-fire_frame_overhead;
		Last_omega_fire_time = GameTime;
	}
...
void Laser_do_weapon_sequence(object *obj)
{
	...
	if (Weapon_info[obj->id].homing_flag && !(obj->id==GUIDEDMISS_ID && obj->ctype.laser_info.parent_type==OBJ_PLAYER && obj==Guided_missile[Objects[obj->ctype.laser_info.parent_num].id] && obj->signature==Guided_missile[Objects[obj->ctype.laser_info.parent_num].id]->signature))
	{
		...
		if (obj->ctype.laser_info.creation_time + HOMING_FLY_STRAIGHT_TIME < GameTime) {
...
int do_laser_firing_player(void)
{
	...
	int weapon_index = Primary_weapon_to_weapon_info[Primary_weapon];
	...
	while (Next_laser_fire_time <= GameTime) {
		if	((plp->energy >= energy_used) && (primary_ammo >= ammo_used)) {
			...
			if (GameTime - Next_laser_fire_time <= FrameTime) // if firing is prolonged by FrameTime overhead, let's try to fix that.
				fire_frame_overhead = GameTime - Next_laser_fire_time;

                        Last_laser_fired_time = GameTime;

			if (Laser_rapid_fire!=0xBADA55)
				Next_laser_fire_time = GameTime + Weapon_info[weapon_index].fire_wait - fire_frame_overhead;
			else
				Next_laser_fire_time = GameTime + (F1_0/25) - fire_frame_overhead;
...
void do_missile_firing(int drop_bomb)
{
	...
	if (GameTime - Next_missile_fire_time <= FrameTime) // if firing is prolonged by FrameTime overhead, let's try to fix that.
		fire_frame_overhead = GameTime - Next_missile_fire_time;
	...
	if (Guided_missile[Player_num] && Guided_missile[Player_num]->signature==Guided_missile_sig[Player_num]) {
		...
		Next_missile_fire_time = GameTime + Weapon_info[Secondary_weapon_to_weapon_info[weapon]].fire_wait - fire_frame_overhead;
		return;
	}
	...
	if (!Player_is_dead && (Players[Player_num].secondary_ammo[weapon] > 0))	{
		...
		if (Laser_rapid_fire!=0xBADA55)
			Next_missile_fire_time = GameTime + Weapon_info[weapon_index].fire_wait - fire_frame_overhead;
		else
			Next_missile_fire_time = GameTime + (F1_0/25) - fire_frame_overhead;
