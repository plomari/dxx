...
static int trace_segs(const vms_vector *p0, int oldsegnum, int recursion_count, struct segment_bit_array *visited)
{
	int centermask;
	segment *seg;
	fix side_dists[6];
	...
	if (SEGMENT_BIT_ARRAY_GET(visited, oldsegnum))
		return -1;
	SEGMENT_BIT_ARRAY_SET(visited, oldsegnum);
