...
#define PRIMARY_W_TEXT_Y		HUD_SCALE_Y(HIRESMODE?378:157)
#define PRIMARY_AMMO_X			HUD_SCALE_X(HIRESMODE?186:93)
#define PRIMARY_AMMO_Y			HUD_SCALE_Y(HIRESMODE?407:171)
...
void add_points_to_score(int points)
{
	...
	if (Players[Player_num].score/EXTRA_SHIP_SCORE != prev_score/EXTRA_SHIP_SCORE) {
		int snd;
		Players[Player_num].lives = SAT_ADD_UNSIGNED(Players[Player_num].lives, Players[Player_num].score/EXTRA_SHIP_SCORE - prev_score/EXTRA_SHIP_SCORE);
...
void sb_draw_afterburner()
{
	...
	erase_height = HUD_SCALE_Y(fixmul((f1_0 - Afterburner_charge),SB_AFTERBURNER_GAUGE_H));
	gr_setcolor( 0 );
	for (i=0;i<erase_height;i++)
...
void hud_show_kill_list()
{
	...
	for (i=0;i<n_players;i++) {
		...
		else if (Game_mode & GM_BOUNTY && player_num == Bounty_target && GameTime & 0x10000)
			strcpy(name,"[TARGET]");
		else
