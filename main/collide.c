...
static int effect_parent_is_guidebot(const object *effect)
{
	...
	if (robot->signature != effect->ctype.laser_info.parent_signature)
		return 0; /* parent replaced, no idea what it once was */
	return Robot_info[robot->id].companion;
}
...
void maybe_drop_secondary_weapon_egg(object *playerobj, int weapon_index, int count)
{
	...
	if (Players[playerobj->id].secondary_weapon_flags & weapon_flag) {
		...
		max_count = PlayerCfg.ExtendedAmmoRack ? count : min(count, 3);
...
void drop_missile_1_or_4(object *playerobj,int missile_index)
{
	...
	if (num_missiles > 10 && !PlayerCfg.ExtendedAmmoRack)
		num_missiles = 10;
...
static vms_vector find_exit_direction(vms_vector result, object *objp, segment *segp)
{
	for (unsigned side = MAX_SIDES_PER_SEGMENT; side --;)
		if (WALL_IS_DOORWAY(segp, side) & WID_FLY_FLAG) {
			...
			vm_vec_sub2(&exit_point, &objp->pos);
			vm_vec_normalize_quick(&exit_point);
			vm_vec_add2(&result, &exit_point);
			break;
		}
	return result;
}
...
void collide_player_and_materialization_center(object *objp)
{
	...
	rand_vec.y /= 4;
	rand_vec.z /= 4;
	vms_vector exit_dir = find_exit_direction(rand_vec, objp, segp);
...
void collide_robot_and_materialization_center(object *objp)
{
	...
	vms_vector exit_dir = find_exit_direction((vms_vector){0}, objp, segp);
